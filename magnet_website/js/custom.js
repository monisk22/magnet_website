...
optionarr=(`
    ...
    <optgroup label="Other countries">
        ...
        <option data-countrycode="CW" value="+599">CuraÃƒÆ’Ã‚Â§ao (+599)</option>
        <option data-countrycode="CI" value="+225">Ivory Coast (+225)</option>
        <option data-countrycode="XK" value="+383">Kosovo (+383)</option>                              
    `)

let selectoption= document.querySelectorAll('[data-dropdown-options]')
 Array.from(selectoption).forEach(item=>{
   item.innerHTML=optionarr
 })
    ...
    let carousel_id = document.getElementById("carousel-id")
    let breakpoint = window.matchMedia('(min-width: 992px)');
    let carousel_item = document.querySelectorAll('#carousel-id .carousel-item')

    if (breakpoint.matches) {
        carousel_id.classList.remove('carousel-inner')
        carousel_id.classList.add('flex')
        // carousel_item.classList.remove('carousel-item');
        for (let i = 0; i < carousel_item.length; i++) {
            carousel_item[i].classList.remove('carousel-item');
        }
    }

    //code for app feature
    let video_list = document.getElementById('video_list');
    console.log(video_list);


    function changeVideo(video = null) {
        if (video) {
            video_list.setAttribute('src', `img/video/${video}.webm`);
        }
    }
    ...
    function showSlides(n) {
        ...
        let slide_btn = document.getElementsByClassName("slide_btn");
        ...
        for (i = 0; i < slide_btn.length; i++) {
            slide_btn[i].className = slide_btn[i].className.replace(" active", "");
        }
        slides[slideIndex - 1].style.display = "block";
        slide_btn[slideIndex - 1].className += " active";
    }
